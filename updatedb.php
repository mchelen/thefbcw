...
//update to change date values to unix timestamps
else if ($whatup == 'fixdates')
{

$sql1 = sprintf('SELECT * FROM `%s`.`TransactionHistory`',$dbname);
$result1 = mysql_query($sql1);
$i=0;
		while ($row1 = mysql_fetch_array($result1, MYSQL_ASSOC))
			{
			$newStamp[$row1['thRef']]=strtotime($row1['thDate']);

			}	

	$sql3 = 'ALTER TABLE `TransactionHistory` CHANGE `thDate` `thDate` INT(11) NOT NULL DEFAULT \'0\'';		
...
$i=0;
		while ($row1 = mysql_fetch_array($result1, MYSQL_ASSOC))
			...
			$sql2 = sprintf('UPDATE `%s`.`TransactionHistory` SET `thDate` = \'%s\' WHERE `TransactionHistory`.`thRef` =%d LIMIT 1 ;'
			,$dbname
			,$newStamp[$row1['thRef']]
			,$row1['thRef']);
...
//update to change daily rates to an apr
else if ($whatup == 'rateapr')
{
$sql1 = sprintf('SELECT * FROM `%s`.`BankAccounts`',$dbname);
$result1 = mysql_query($sql1);
		...
		while ($row1 = mysql_fetch_array($result1, MYSQL_ASSOC))
			...
			$newRate[$row1['baKey']]=exp(log(1+$row1['baInterestRate'])/(86400/$compoundPeriodSeconds))-1;
			
			echo "<br />account ".$row1['baKey'].": exp(log(1+".$row1['baInterestRate'].")/(86400/$compoundPeriodSeconds))-1 = ".$newRate[$row1['baKey']];
...
$sql = 'ALTER TABLE `BankAccounts` CHANGE `baInterestRate` `baInterestRate` FLOAT(24,12) NULL DEFAULT NULL';
...
$result1 = mysql_query($sql1);
		...
		while ($row1 = mysql_fetch_array($result1, MYSQL_ASSOC))
			...
			$sql2 = sprintf('UPDATE `%s`.`BankAccounts` SET `baInterestRate` = \'%f\' WHERE `BankAccounts`.`baKey` =%d LIMIT 1 ;'
			,$dbname
			,$newRate[$row1['baKey']]
			,$row1['baKey']);
